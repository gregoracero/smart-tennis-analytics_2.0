...
for player in df["player"].dropna().unique():
    ...
    for surface in player_df["surface"].dropna().unique():
        ...
        for tournament_group, levels in TOURNAMENT_GROUPS.items():

            if levels is None:
                tournament_df = surface_df.copy()

            else:
                tournament_df = tournament_df[
                    tournament_df["tourney_level"].isin(levels)
                ]
            ...
            for window_name, cutoff in WINDOWS.items():
                ...
                if window_name == "LAST_10_MATCHES":
                    ...
                    subset = tournament_df[
                        tournament_df["tourney_date"] >= cutoff
                    ]
